...
import { fetchVault, upsertVault, deleteVault } from './cloudStore'
...
// ── 로컬 캐시 키 (오프라인 폴백 / 빠른 재진입용) ────────────────
const LOCAL_CACHE_KEY = 'finance_v6_cache'
...
interface Insurance {
  ...
  amount: number; joinDate: string; payExpire: string; coveragePeriod: string
  tags: Tags; payment: PaymentMethod
}
...
// ── 루트: 인증 + 잠금 + 클라우드 동기화 ─────────────────────────
type AppState = 'loading' | 'needs-auth' | 'needs-setup' | 'locked' | 'unlocked'
...
export default function FinanceApp() {
  ...
  // 1) Supabase 세션 확인 → 있으면 vault 체크, 없으면 로그인 화면
  const checkVaultAfterAuth = useCallback(async (uid: string) => {
    if (!isSupabaseConfigured) { setAppState('needs-setup'); return }
    setSyncStatus('syncing')
    const vault = await fetchVault(uid)
    setSyncStatus(vault ? 'synced' : 'idle')
    setAppState(vault ? 'locked' : 'needs-setup')
  }, [])

  useEffect(() => {
    if (!isSupabaseConfigured) {
      // Supabase 환경변수가 없으면 로컬 전용 모드로 동작 (개발/오프라인 대비)
      try {
        const cached = localStorage.getItem(LOCAL_CACHE_KEY)
        setAppState(cached ? 'locked' : 'needs-setup')
      } catch {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      setSession(session)
      if (session) checkVaultAfterAuth(session.user.id)
      else setAppState('needs-auth')
    })

    const { data: listener } = supabase.auth.onAuthStateChange((_event, newSession) => {
      setSession(newSession)
      if (newSession) {
        checkVaultAfterAuth(newSession.user.id)
      } else {
        setAppState('needs-auth')
        setPassword(null)
        setData(EMPTY)
      }
    })
  ...
  // 2) 데이터 저장: 로컬 캐시(암호문) + Supabase 동시 반영
  const save = useCallback(async (d: AppData) => {
    setData(d)
    if (!password) return
    try {
      const payload = await encryptData(JSON.stringify(d), password)
      try { localStorage.setItem(LOCAL_CACHE_KEY, JSON.stringify(payload)) } catch { /* ignore */ }

      if (isSupabaseConfigured && session) {
        setSyncStatus('syncing')
        const ok = await upsertVault(session.user.id, payload)
  ...
  // 잠금 해제: Supabase에서 vault를 먼저 받아오고, 없으면 로컬 캐시, 둘 다 없으면 최초 설정으로
  const handleUnlock = async (pw: string): Promise<boolean> => {
    try {
      ...
      if (isSupabaseConfigured && session) {
        setSyncStatus('syncing')
        payload = await fetchVault(session.user.id)
        setSyncStatus(payload ? 'synced' : 'idle')
      }
      if (!payload) {
  ...
  const handleForgot = async () => {
    ...
    if (isSupabaseConfigured && session) {
      await deleteVault(session.user.id)
    }
    setData(EMPTY)
    setPassword(null)
    setAppState('needs-setup')
  ...
  useEffect(() => {
    ...
    evs.forEach(ev => window.addEventListener(ev, markActivity))
  ...
  if (appState === 'loading') return <div className="lock-wrap" />
  if (appState === 'needs-auth') return <AuthScreen onAuthed={() => { /* onAuthStateChange가 처리 */ }} />
  if (appState === 'needs-setup') return <SetupLock onSet={handleSetup} />
  if (appState === 'locked') {
    return (
      <Unlock
  ...
  return <MainApp data={data} save={save} onLock={lockNow} syncStatus={syncStatus} cloudEnabled={isSupabaseConfigured} />
}

// ── 메인 앱 ───────────────────────────────────────────────────
function SyncBadge({ status, cloudEnabled }: { status: SyncStatus; cloudEnabled: boolean }) {
  if (!cloudEnabled) return <span className="sync-badge sync-offline">로컬 전용</span>
  const map: Record<SyncStatus, { label: string; cls: string }> = {
    idle:     { label: '대기', cls: 'sync-idle' },
    syncing:  { label: '동기화 중...', cls: 'sync-syncing' },
    synced:   { label: '☁️ 동기화됨', cls: 'sync-synced' },
    error:    { label: '⚠️ 동기화 실패', cls: 'sync-error' },
    offline:  { label: '오프라인', cls: 'sync-offline' },
  }
  const m = map[status]
  return <span className={`sync-badge ${m.cls}`}>{m.label}</span>
}

function MainApp({ data, save, onLock, syncStatus, cloudEnabled }: {
  data: AppData; save: (d: AppData) => void; onLock: () => void
  syncStatus: SyncStatus; cloudEnabled: boolean
}) {
    ...
    <div className="app-wrap">
      <div className="app-header">
        <div>
          ...
          <SyncBadge status={syncStatus} cloudEnabled={cloudEnabled} />
        </div>
...
function buildEmptyForm(typeDefault: string) {
  return {
    name: '', type: typeDefault, amount: '', extra: '', tags: '',
    payment: { ...EMPTY_PAYMENT } as PaymentMethod,
    joinDate: '', payExpire: '', coveragePeriod: '', // 보험 전용 (구독/교육에서는 사용하지 않음)
  }
}
...
// ── 보험 탭 (가입일/납입만료일/보험기간 포함) ──────────────────
function InsuranceForm({ form, setForm, onSave, onCancel, saveLabel }: {
  form: PayFormState; setForm: (f: PayFormState) => void
  onSave: () => void; onCancel: () => void; saveLabel: string
}) {
  return (
    <>
      <div className="form-grid g2">
        <div className="form-field"><label>보험명</label><input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="예: 실손의료보험" /></div>
        <div className="form-field"><label>카테고리</label>
          <select value={form.type} onChange={e => setForm({ ...form, type: e.target.value })}>
            {INSURANCE_TYPES.map(o => <option key={o}>{o}</option>)}
          </select>
        </div>
      </div>
      <div className="form-grid g2">
        <div className="form-field"><label>월 보험료 (원)</label><input type="number" value={form.amount} onChange={e => setForm({ ...form, amount: e.target.value })} placeholder="0" /></div>
        <div className="form-field"><label>보험사</label><input value={form.extra} onChange={e => setForm({ ...form, extra: e.target.value })} placeholder="예: 삼성생명" /></div>
      </div>
      <div className="form-grid g3">
        <div className="form-field"><label>가입일</label><input type="date" value={form.joinDate} onChange={e => setForm({ ...form, joinDate: e.target.value })} /></div>
        <div className="form-field"><label>납입만료일</label><input type="date" value={form.payExpire} onChange={e => setForm({ ...form, payExpire: e.target.value })} /></div>
        <div className="form-field"><label>보험기간</label><input value={form.coveragePeriod} onChange={e => setForm({ ...form, coveragePeriod: e.target.value })} placeholder="예: 100세 만기" /></div>
      </div>
      <div className="divider-label" style={{ marginTop: 4 }}>결제수단</div>
      <PaymentFields value={form.payment} onChange={p => setForm({ ...form, payment: p })} />
      <div className="form-grid g1" style={{ marginTop: 9 }}>
...
function InsuranceTab({ data, save }: { data: AppData; save: (d: AppData) => void }) {
  ...
  const add = () => {
    const item: Insurance = {
      id: Date.now(), name: form.name || '미입력', company: form.extra, itype: form.type,
      amount: +form.amount || 0, joinDate: form.joinDate, payExpire: form.payExpire, coveragePeriod: form.coveragePeriod,
      tags: parseTags(form.tags), payment: form.payment,
    }
  ...
  const startEdit = (ins: Insurance) => {
    setEditId(ins.id)
    setEditForm({
      name: ins.name, type: ins.itype, amount: String(ins.amount), extra: ins.company,
      tags: tagsToStr(ins.tags), payment: ins.payment ?? { ...EMPTY_PAYMENT },
      joinDate: ins.joinDate ?? '', payExpire: ins.payExpire ?? '', coveragePeriod: ins.coveragePeriod ?? '',
    })
  }
  const saveEdit = (id: number) => {
    const updated: Insurance = {
      id, name: editForm.name || '미입력', company: editForm.extra, itype: editForm.type,
      amount: +editForm.amount || 0, joinDate: editForm.joinDate, payExpire: editForm.payExpire, coveragePeriod: editForm.coveragePeriod,
      tags: parseTags(editForm.tags), payment: editForm.payment,
    }
  ...
  return (
    <>
      <div className="toolbar">
        ...
        <button className="icon-btn" onClick={() => downloadCSV(`보험_${todayStr()}.csv`, [['보험명','보험사','종류','월보험료','가입일','납입만료일','보험기간','결제수단','결제일','태그'], ...data.insurance.map(i => [i.name, i.company, i.itype, i.amount, i.joinDate ?? '', i.payExpire ?? '', i.coveragePeriod ?? '', paymentSummary(i.payment ?? EMPTY_PAYMENT), i.payment?.payDay ?? '', i.tags.join(';')])])}>↓ CSV</button>
      </div>
      <FormBox open={open}>
        <InsuranceForm form={form} setForm={setForm} onSave={add} onCancel={() => setOpen(false)} saveLabel="저장" />
      </FormBox>
      <div className="card">
        {data.insurance.length === 0
          ...
          : <>
              {data.insurance.map(ins => {
                if (editId === ins.id) {
                  return <div key={ins.id} className="edit-row-box"><InsuranceForm form={editForm} setForm={setEditForm} onSave={() => saveEdit(ins.id)} onCancel={() => setEditId(null)} saveLabel="수정 완료" /></div>
                }
                const pay = ins.payment ?? EMPTY_PAYMENT
                const dateInfo = [
                  ins.joinDate ? '가입 ' + ins.joinDate : '',
                  ins.payExpire ? '납입만료 ' + ins.payExpire : '',
                  ins.coveragePeriod ? '보험기간 ' + ins.coveragePeriod : '',
                ].filter(Boolean).join(' · ')
                return (
                  <div key={ins.id} className="row">
                    <div>
                      <div className="row-name">{ins.name} <Badge t={ins.itype} /></div>
                      <div className="row-meta">{ins.company}{dateInfo ? ' · ' + dateInfo : ''}{paymentSummary(pay) ? ' · ' + paymentSummary(pay) : ''} <TagList tags={ins.tags} /></div>
                    </div>
...
function SearchTab({ data }: { data: AppData }) {
  ...
  if (q) {
    ...
    if (!cat || cat === 'insurance')    data.insurance.filter(i => hit(`${i.name} ${i.company} ${i.itype} ${i.tags.join(' ')} ${paymentSummary(i.payment ?? EMPTY_PAYMENT)} ${i.joinDate ?? ''} ${i.payExpire ?? ''} ${i.coveragePeriod ?? ''}`)).forEach(i => results.push({ type: 'insurance', label: TL.insurance, name: i.name, detail: `${fmtW(i.amount)}/월 · ${i.company}`, tags: i.tags }))
